...
public:
	...
	virtual void onCreate(Application& app) override {
		...
		renderer->ambient(Vector3(0.0f, 0.05f, 0.15f));
		...
		mcomp->material().texture(Material::SlotDiffuse, ResourceManager::ston().load<Texture>("diff", "diff.png"));
		mcomp->material().texture(Material::SlotNormal, ResourceManager::ston().load<Texture>("norm", "norm.png"));
		mcomp->material().texture(Material::SlotHeight, ResourceManager::ston().load<Texture>("spec", "spec.png"));
		mcomp->material().shininess(0.8f);
		mcomp->material().specular(1.0f);

		bcomp->material().texture(Material::SlotDiffuse, ResourceManager::ston().load<Texture>("ref", "bunny_diff.png"));
		bcomp->material().specular(1.0f);
		bcomp->material().shininess(0.5f);

		Entity* light = world->create();
		light->position(Vector3(6.0f));
		auto lcomp = light->createComponent<LightComponent>();
		lcomp->type(LightType::Directional);
		lcomp->color(Vector3(1.0f));
		light->rotation(Quaternion::lookAt(light->position(), Vector3(0.0f), Vector3(0.0f, 1.0f, 0.0f)));
		...
		// lcomp2->color(Vector3(0.0f, 0.25f, 1.0f));
